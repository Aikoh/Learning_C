...
ノーマル変数という名前は説明の便宜上ここで使っているだけで、普通の「変数」として扱える状態の事を表してます。

---
### ポインタ変数とノーマル変数の切り替え

特に何もぜずにポインタ変数を使っている場合はポインタ変数として扱われます。

ノーマル変数として使う場合は、変数の前に*をつける事で通常つくった時の変数と同じ扱い方ができます。
...
int *pointa;
```
この状態だと変数宣言直後に適当なアドレスが
入ってしまう為、そのアドレスが使える保障がない

```
int *pointa = NULL;
```
NULLを代入する事でpointa == NULLとする事で
pointaにアドレスが代入されているか、否かが判別
できる。
...
```
---
### 問題２ 次の仕様を満たしたカードゲームのデッキの勝敗をシミュレートするプログラムを作りなさい。
...
2. 自分のデッキ5枚分のスキルを選ぶ
3. 5枚のデッキを選択する
4. スキルは、１、特大UP、２大UP、３特大Down、４大Downの４種類ある
5. 特大UPの係数は2倍、大UPは1.5倍、特大Downは0.2倍減、大Downは0.15倍減
6. 自分と相手のデッキはコンパイル時にランダムで5枚セットされる。
7. バトルスタートをさせる事で勝敗を表示する。演出をすること。
8. 演出方法に関しては自由とする。
